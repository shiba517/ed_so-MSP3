...
--- | --- | --- | --- | --- |
Can a user create a recipe? | yes  | yes
Can a user read/view a recipe? | yes | yes
Can a user update a recipe? | yes | yes
Can a user delete a recipe? | yes | yes

### Home page testing (home.html)
AIM | Mobile | Tablet
--- | --- | ---|
Any user clicking on 'more' button will lead to a smoothe scrolldown to the 'how it works' section of the homepage | yes | yes
Any user clicking on either 'register' buttons will lead to the register.html | yes | yes
Any user clicking on 'login' button will lead to login.html | yes | yes
'register' button will not appear when a user is logged in | yes | yes

### Navbar testing (base.html)
AIM | Mobile | Tablet
--- | --- | ---|
Clicking on the logo will lead user to the home.html | yes | yes
When user is not logged in, only link that will appear will be 'Home', 'All recipes', 'Login', and 'Register' | yes | yes
When user is logged in, the links on show will be 'Home, 'All recipes', 'Random', 'Profile', and 'Logout' | yes | yes
Clicking on 'logout' will lof the user out if his/her account and back to home.html | yes | yes
If log out is successful, a flash should appear with the text "You have logged out" | yes | yes

### Register testing (register.html)
AIM | Mobile | Tablet
--- | --- | ---|
'username' will only take in aplabetical characters(upper and lower case) and numbers | yes | yes
'username' will only take upto 20 characters | yes | yes
'username' will take a minimum of 5 characters | yes | yes
'password' will only take in aplabetical characters(upper and lower case) and numbers | yes | yes
'password' will take a minimum of 5 characters | yes | yes
'password' will only take upto 20 characters | yes | yes
Clicking on 'login from here' will take the user to the login.html | yes | yes
Clicking on 'sign up' will update the intended mongo database (mongo.db.users) | yes | yes
Clicking on 'sing up' will also taek the user to their profile page (profile.html) | yes | yes
...
'password' will take a minimum of 5 characters | yes | yes
'password' will only take upto 20 characters | yes | yes
Clicking on 'register from here' will take the user to the login.html | yes | yes
Clicking on 'login' will also take the user to their profile page (profile.html) if successful | yes | yes
If clicking on 'login' and app.py did not find a match in the details entered, a flash message of "Username and/or password is incorrect" will appear | yes | yes
If clicking on 'login' and app.py did not find a match in the details entered, details entered will be cleared from the form | yes | yes

### Profile testing (profile.html)
AIM | Mobile | Tablet
--- | --- | ---|
Username of the user will appear near the top of the page | yes | yes
Buttons of 'Add', 'Mine', 'Random' and 'Bye' will appear on screen | yes | yes
Clicking on 'add' will take user to the add_recipe.html | yes | yes
Clicking on 'mine' will take user to the my_recipes.html | yes | yes
Clicking on 'random' will take user to the random_recipes.html | yes | yes
Clicking on 'bye' will lead to a modal appearing | yes | yes
The modal will ask 'Delete?' with a follow message 'Clicking yes will permanently delete your account and all your recipes' | yes | yes
Clicking on 'no' will remove the modal | yes | yes
Clicking on 'yes' will delete the user from the intended monog database (mongo.db.users) | yes | yes
Clicking on 'yes' will also lead the user back to the home.html | yes | yes
A flash message of "You are no longer one of us" should appear | yes | yes
A card of the most popular recipe based on the ammount of likes will also be on show | yes | yes
...
Quantity of cards on show will match the quantity of entries in the mongo database that it is coming from | yes | yes
There should be three cards on each row for large+ sized screen | n/a | n/a 
There should be two cards on each row for medium sized screen | na | yes
There should be one card on each row in a small sized screen | yes | n/a
Clicking on the 'i' icon should lead to another card sliding up contained the name of the recipe and the description | yes | yes
Clicking on the 'i' icon when the sliding happens should slide it back down | yes | yes
'lets make this' should not be visible if the user is not logged in | yes | yes
If logged in, 'lets make this' should be visible | yes | yes
Clicking on 'lets make this' will lead teh user to chosen_recipe.html with detials of the chosen card | yes | yes

### Chosen recipes
AIM | Mobile | Tablet
--- | --- | ---|
Image of recipe should be near the top | yes | yes
Recipe name, username, creation date, likes, servings, cooking curation, and recipe description should appear near the upper right of the screen if on a large+ sized screen | n/a | n/a
If nothing has been entered for the recipes image url, a [default image](static/images/default_recipe_image.jpg) should appear | yes  | yes
Recipe name, username, creation date, likes, servings, cooking curation, and recipe description should appear one below the other in that order on a medium- sized screen | yes  | yes
Clicking on the 'like' icon (heart) will increment the number below it by 1 | yes | yes
Recipe ingredients list should appear near the lower left of the screen on a large+ sized screen | n/a | n/a
Recipe instructions list should appear near the lower right of the screen on a large+ sized screen | n/a | n/a
Recipe ingredints will appear in an unordered list format | yes | yes
Recipe instructions will appear in an ordered list format | yes | yes
Bottom corner of the screen should have a 'plus' icon if the recipes was posted by the user logged in | yes | yes
Hovering on the 'plus' icon will lead to the icons of 'delete' and 'edit' to appear if on a large+ sized screen | n/a | n/a
Clicking on the 'plus' icon will lead to the icons of 'delete' and 'edit' to appear if on a medium+ sized screen | yes | yes
Clicking on the 'edit' icon will lead user to the edit_recipe.html | yes
Clicking on 'delete' icon will lead to a modal appearing | yes | yes
The modal will ask 'Delete?' with a follow message 'Clicking yes will permanently delete this recipe' | yes | yes
Clicking on 'no' will remove the modal | yes | yes
Clicking on 'yes' will delete the recipe from the intended monog database (mongo.db.recipes) | yes | yes
clicking on 'yes' will also lead the user back to the all_recipes.html | yes | yes

### Edit recipe
AIM | Mobile | Tablet
--- | --- | ---|
Each field on the form should match the information on the mongo databse of the recipe chosen to edit | yes | yes
Clicking on any field should give the user the ability to change the inputs | yes | yes
Clicking on 'edit' will update the informatino of that recipe in the mongo database | yes | yes
Clicking on 'edit' will also then lead the user to the all_recipes.html | yes | yes
Changed information of edited recipe should be visible in ohter pages the recipe appears in (all_recipes.html, chosen_recipes.html, and my_recipes.html) | yes | yes

### Add recipe testing (add_recipe.html)
AIM | Mobile | Tablet
--- | --- | ---|
'recipe name' field should take in any characters | yes | yes
'recipe name' will take upto 100 characters in length | yes | yes
'recipe description' field will take in any characters | yes | yes
'recipe description' field size should accomodate the entered information | yes | yes
'recipe ingredients' field will take in any characters | yes | yes
'recipe ingredients' field size should accomodate the entered information | yes | yes
'recipe instructions' field will take in any characters | yes | yes
'recipe instructions' field size should accomodate the entered information | yes | yes
'recipe image url' field will take in url text | yes | yes
'recipe imahe url' field size should accomodate the entered information | yes | yes
'recipe image url' is not a required field | yes | yes
'servings' will only take in numbers | yes | yes
'servings' will take in a number of maximum of 2 digits long | yes | yes
'servings' will take in a number of minimum of 1 digit | yes | yes
'duration' will only take in numbers | yes | yes
'duration' will take in a number of maximum of 3 digits long | yes | yes
'duration' will take in a number of minimum of 1 digit | yes | yes
Clicking on 'share' will update the relevant mongo database | yes | yes
Clicking on 'share' will also lead teh user to the all_recipes.html | yes | yes
Flash message of "Recipe has been added" should appear | yes | yes

### Edit recipe testing (edit_recipe.html)
AIM | Mobile | Tablet
--- | --- | ---|
All fields should be prefilled witht he details of the chosen recipe to edit | yes | yes
Clicking on the 'edit' button should update the details of that recipe on the mongo database with the new/edited details | yes | yes
Clicking on the 'edit' button should lead the user to the my_recipe.html | yes | yes
Flash message of "Recipe hsa been updated" should appear | yes | yes

### My recipes testing
AIM | Mobile | Tablet
--- | --- | ---|
All recipe cards on show will be recipes posted by the user | yes | yes
There should be three cards on each row for large+ sized screen | n/a | n/a 
There should be two cards on each row in a medium sized screen | yes | yes
There should be one card on each row in a small- sized screen | yes | n/a

### Random recipes testing
AIM | Mobile | Tablet
--- | --- | ---|
Three cards will be on show | yes | yes
There should be three cards on each row for large+ sized screen | n/a | n/a 
There should be one card on each row in a medium- sized screen | yes | yes
Recipe cards will change each time random.html is clicked | yes

### Admin testing
AIM | Mobile | Tablet
--- | --- | ---|
Ability to edit any recipe | yes | yes
Ability to delete any recipe | yes | yes
'bye' button will not be visible in profile.html | yes | yes
'ban' button will be visible in profile.html | yes | yes
On ban.html, entered user will be deleted from teh mongo database | yes | yes
The deleted user will also have all theri recipes deleted from the mongo database | yes | yes
A flash message of "that chef has been fired" should appear | yes | yes
A flash message of "user does not exist" should appear if user does not exist | yes | yes
The form should be cleared of previous entry | yes | yes

### Security testing
AIM | Mobile | Tablet
--- | --- | ---|
When logging out, session cookie of user will be deleted | yes | yes
If session cookie of the user is not in storage when navigating to the profile.html, page will lead to the error401.html | yes | yes
If session cookie of the user is not in storage when navigating to the all_recipes.html, page will load up the same way as non logged in user would see it | yes | yes
If session cookie of the user is not in storage when navigating to the random_recipes.html, page will lead to the error404.html | yes | yes
If session cookie of the user is not in storage when navigating to the my_recipes.html, page will lead to the error401.html | yes | yes
If session cookie of the user is not in storage when navigating to the chosen_recipe.html, page will lead to the error401.html | yes | yes
If session cookie of the user is not in storage when navigating to the chosen_recipe.html, page will lead to the error401.html | yes | yes
If session cookie of the user is not in storage when navigating to the edit_recipe.html, page will lead to the error401.html | yes | yes
If session cookie of the user is not in storage when clicking 'edit' on the edit_recipe.html, page will lead to the error404.html | yes | yes
If details of the session cookie of the user does not match the chosen recipe for editing, page will lead to the error401.html | yes | yes
If session cookie of the user is not in storage when navigating to the add_recipe.html, page will lead to the error401.html | yes | yes
If session cookie of the user is not in storage when clicking 'share' on the edit_recipe.html, page will lead to the error404.html | yes | yes
If session cookie of the user is not in storage when clicking 'bye' > 'yes' in the profile.html, page will lead to the error404.html | yes | yes
Relevant MongoDB database will not be updated when this happens | yes | yes
If session cookie of the admin is not in storage when clicking 'ban' > 'yes' in the ban.html, page will lead to the error404.html | yes | yes
